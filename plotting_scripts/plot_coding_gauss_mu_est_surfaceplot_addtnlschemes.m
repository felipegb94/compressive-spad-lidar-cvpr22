...
out_dirpath = fullfile('../', io_dirpaths.paper_results_dirpath, '/MAESimulationsAddtnlSchemes');
...
ncodes = 8;
coding_ids = {'TruncatedFourier','PSeriesFourier', 'WalshHadamard' 'GatedFourier-F-1'};
rec_algo_ids = {'ncc-irf', 'ncc-irf', 'ncc-irf', 'ncc-irf'};    
...
[min_plot_sbr, max_plot_sbr, min_plot_nphotons, max_plot_nphotons] = get_plot_snr_settings(is_high_flux, is_high_sbr, is_photonstarved);
...
function [min_plot_sbr, max_plot_sbr, min_plot_nphotons, max_plot_nphotons] = get_plot_snr_settings(is_high_flux, is_high_sbr, is_photonstarved)



    if(is_photonstarved)
        min_plot_nphotons = 10;
        max_plot_nphotons = 50;
        if(is_high_sbr)
            min_plot_sbr = 1.0;
            max_plot_sbr = 10.0;
        else
...
function [] = set_zlim(is_high_flux, is_high_sbr, is_narrow_pulse, n_codes)
    if((is_high_flux && is_high_sbr && is_narrow_pulse) && (n_codes==8)) 
        % Used for main paper
%         zlim([0,5])
        zlim([0,4])
    elseif((is_high_flux && is_high_sbr && is_narrow_pulse) && (n_codes==16))
        zlim([0,2])
    elseif((is_high_flux && is_high_sbr && is_narrow_pulse) && (n_codes==32))
        zlim([0,1])
    elseif((is_high_flux && is_high_sbr && is_narrow_pulse) && (n_codes==64))
        zlim([0,0.5])
    elseif((is_high_flux && is_high_sbr && is_narrow_pulse) && (n_codes==128))
        zlim([0,0.5])
    end
end
